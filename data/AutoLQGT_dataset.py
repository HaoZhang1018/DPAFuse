...
class AutoLQGTDataset(data.Dataset):
    """
    ...
    def __getitem__(self, index):
        ...
        if self.opt["phase"] == "train":
            ...
            if H_GT < GT_size or W_GT < GT_size:
                scale_GT = max(GT_size / H_GT, GT_size / W_GT)
                new_H_GT = math.ceil(H_GT * scale_GT)
                new_W_GT = math.ceil(W_GT * scale_GT)
                img_GT = cv2.resize(img_GT, (new_W_GT, new_H_GT), interpolation=cv2.INTER_LINEAR)
        
        
                scale_LR = scale_GT
                new_H_LR = math.ceil(H_LR * scale_LR)
                new_W_LR = math.ceil(W_LR * scale_LR)
                img_LR = cv2.resize(img_LR, (new_W_LR, new_H_LR), interpolation=cv2.INTER_LINEAR)
            ...
            assert LR_size <= H_LR and LR_size <= W_LR, f"LR image too small: {(H_LR, W_LR)} < {LR_size}"
            assert GT_size <= H_GT and GT_size <= W_GT, f"GT image too small: {(H_GT, W_GT)} < {GT_size}"            
            
            
            # randomly crop
            rnd_h = random.randint(0, max(0, H_LR - LR_size))
            rnd_w = random.randint(0, max(0, W_LR - LR_size))
