...
def parse(opt_path, is_train=True):
    ...
    for phase, dataset in opt["datasets"].items():
        ...
        is_lmdb = False
        if dataset.get("dataroot_X", None) is not None:
            dataset["dataroot_X"] = osp.expanduser(dataset["dataroot_X"])
            if dataset["dataroot_X"].endswith("lmdb"):
                is_lmdb = True
        # if dataset.get('dataroot_GT_bg', None) is not None:
        #     dataset['dataroot_GT_bg'] = osp.expanduser(dataset['dataroot_GT_bg'])
        if dataset.get("dataroot_Y", None) is not None:
            dataset["dataroot_Y"] = osp.expanduser(dataset["dataroot_Y"])
            if dataset["dataroot_Y"].endswith("lmdb"):
                is_lmdb = True
